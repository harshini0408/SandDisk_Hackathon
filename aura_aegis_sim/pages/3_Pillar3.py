...
with st.sidebar:
    ...
    inj_block = st.number_input("Block #:", 0, 63, 32, key="inj_block_p3")
    if st.button("💥 Force Bad", key="force_bad_btn_p3"):
        sim.force_bad(int(inj_block))
        st.rerun()

    if st.button("🌡️ Thermal Spike", key="thermal_btn_p3"):
        sim.inject_thermal_spike()
        st.rerun()
    if st.button("⚡ Write Storm", key="storm_btn_p3"):
        sim.inject_write_storm()
        st.rerun()
    if st.button("💀 Kill Host", key="kill_btn_p3"):
        sim.kill_host()
        st.rerun()
...
st.info("**Pillar 3** is the error correction engine. "
        "It reports ECC correction rates **UP** to Pillar 1's SMART metrics, "
        "and receives LDPC ceiling adjustment commands **DOWN** from Pillar 1's LSTM.")
...
with st.expander("📡 Signals sent to Pillar 1 (live)"):
    st.markdown("""
| ECC Event | SMART metric updated | Effect in Pillar 1 |
|---|---|---|
| Tier 2 correction | ① ECC correction rate | Sparkline trends up |
| Tier 3 escalation | ⑨ Controller retry freq | LSTM flags RBER spike |
| UECC (unrecoverable) | ② UECC count +1 | CRITICAL anomaly triggered |
| Syndrome count rising | ⑥ RBER update | Baseline deviation computed |
""")
